...
import { Shield, BrainCircuit, Activity, FileText, FlaskConical, ExternalLink, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-primary/30 selection:text-primary relative font-sans text-foreground">
      ...
      <main className="relative">
        ...
        {/* ── Lab Bench Exhibits ─────────────────────────────────────────── */}
        <section className="py-24 px-6 border-t border-white/5 bg-black relative" id="exhibits">
          <div className="absolute inset-0 pointer-events-none opacity-[0.05] grid-bg" />
          <div className="max-w-6xl mx-auto text-center relative z-10 space-y-6 mb-16">
            ...
            <p className="mt-4 text-[11px] font-mono text-white/40 max-w-xl mx-auto uppercase tracking-[0.2em] leading-relaxed">
            ...
            <Link
              to={`/demo/${engineRegistry[0]?.id}`}
              className="inline-flex items-center gap-2 mt-2 px-6 py-3 bg-primary text-black text-[10px] font-mono uppercase tracking-widest hover:bg-primary/80 transition-colors"
            >
              Start First Exhibit <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
